...
$Tool = Join-Path $ProjectRoot "tools\dev\run_batch10_5_packaged_smoke.py"

foreach ($RequiredFile in @($Python, $Exe, $Tool)) {
    if (-not (Test-Path -LiteralPath $RequiredFile -PathType Leaf)) {
        throw "A required Batch 10.5 packaged-smoke input is unavailable."
    }
}
...
$Runtime = Join-Path $env:TEMP "MusicVault_Batch10_5_PackagedSmoke_$Timestamp"
$Manifest = Join-Path $Runtime "acceptance-manifest.json"
$NetworkReport = Join-Path $Runtime "batch10_5-network-report.json"
$ReviewPlan = Join-Path $Runtime "batch10_5-ui-review-plan.json"
...
try {
    ...
    if ($LASTEXITCODE -ne 0) { throw "Packaged Batch 10.5 verification failed." }
    $Succeeded = $true
}
finally {
    foreach ($Name in $EnvironmentNames) {
    ...
    elseif ($Succeeded -and $GracefulCloseConfirmed -and (Test-Path -LiteralPath $Runtime)) {
        ...
        if (-not $Leaf.StartsWith("MusicVault_Batch10_5_PackagedSmoke_", [StringComparison]::Ordinal)) {
        ...
        if (Test-Path -LiteralPath $ReviewOutput) {
            ...
            if (-not (Split-Path -Leaf $ResolvedReview).StartsWith("MusicVault_Batch10_5_PackagedSmoke_", [StringComparison]::Ordinal)) {
