...
def test_verify_accepts_v4_to_current_migration_with_verified_rollback_backup(
    tmp_path: Path,
):
    data_dir, database, _media = _seed_v4_runtime(tmp_path)
    baseline = gate.capture_baseline(
...
def test_invalid_second_sqlite_file_is_not_accepted_as_automatic_backup(
    ...
    invalid_backup = (
        ...
        / "backups"
        / (
            f"music_vault_pre_schema_v{gate.EXPECTED_SCHEMA_VERSION}_"
            "2099-01-01_00-00-00.sqlite3"
        )
    )
